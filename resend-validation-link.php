...
if(isset($_POST["rwsusername"]))
{
	...
	if(empty($errors)) 
	{
		...
		if($numrows>0)
		{
			...
			$validateid = base64_encode("SS-".$rowp["user_id"]);		
...
We are almost done. just use the link below to validate your account:<br>
<br>
<div style="text-align:center">
<a href="'.$activeurl.'" title="Click Here to Join" style="display:inline-block; background:#2f4f99; border:2px solid #55acee; text-decoration:none; border-radius:10px; padding:10px 25px; text-transform:uppercase; color:#FFF; font-size:16px; font-weight:600; margin:17px 0 0 0;" target="_blank">Validate Account »</a>
</div>
<br>
<br>
...
<br />
  ...
  '.$emailfooter;
		...
		$gt_msgerror = '<div id="rws-formsuccess">An email has been sent with validation link to your registered email id with us. Please check your email. if the problem further persists, Please call us immediately on '.$admin_support_mobile.'.</div>';
	}
	else
	{
		$gt_msgerror = '<div id="rws-formfeedback">Sorry! Your Username/Email id doesn\'t found with our network, please use the correct Username/Email and try once again. if the problem further persists, Please call us immediately on '.$admin_support_mobile.'.</div>';
	}
	}
	else
	{
...
<div class="container rws-contentgap">	
    <div class="row">    
        ...
        <div class="rws-userformdesign">
        	<h1 style="margin-top:0;">Resend Validation Link</h1>
