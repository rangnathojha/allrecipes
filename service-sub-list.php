...
$nquery="";
	...
	if($_GET["service_id"]==1)
	{
		...
		$query="SELECT t1.*, IFNULL(t2.totalsubitems,0) as totalsubitems FROM ss_services as t1 LEFT JOIN (SELECT service_id, parent_id, COUNT(*) AS totalsubitems FROM ss_services GROUP BY service_id ) AS t2 ON t1.service_id = t2.parent_id WHERE t1.service_id > 0 AND t1.status=1 ".$nquery." GROUP BY t1.service_id ";
	...
	<?php if($foundnum>0) { ?>	
        ...
        <div class="row">    
              <?php
				...
				{
					$imgurl = $baseurl.'images/servicebg/'.$row["service_image"];
				}
				else
				{
					$imgurl = $baseurl.'images/home-banner.jpg';
				}
		
					if($row["totalsubitems"]==0)
					{
						echo '<div class="col-sm-3 gtservice_'.$j.'">
		 <div class="gteventlistitems gtcolorblue">
		 <div class="gtlistimg"  style=" background:url('.$imgurl.') no-repeat  center top; background-size:cover; min-height:200px; margin-bottom:0px;">
</div>
				
				<div class="gtlisttitle">'.$row["name"].'</div><div class="gtlistexcert">'.$row["service_description"].'</div>	<div class="gtlistreadmore"><a href="'.$baseurl.'service-details.php?service_id='.$row["service_id"].'">Read More</a></div>			 </div>
		 ...
		 <div class="gtlistimg"  style=" background:url('.$imgurl.') no-repeat  center top; background-size:cover; min-height:200px; margin-bottom:0px;">
				...
				<div class="gtlisttitle">'.$row["name"].'</div><div class="gtlistexcert">'.$row["service_description"].'</div>	<div class="gtlistreadmore"><a href="'.$baseurl.'service-sub-list.php?service_id='.$row["service_id"].'">Read More</a></div>			 </div>
