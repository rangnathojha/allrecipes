...
/* ----- Action Code Ends HERE ----- */
	...
	if($search_txt !="")
	{
		...
		if($x==1)
			$nquery = " AND (t1.package_subtitle LIKE '%$search_txt%' OR t3.name LIKE '%$search_txt%') ";
		else
			$nquery = " AND (t1.package_subtitle LIKE '%$search_txt%' OR t3.name LIKE '%$search_txt%') ";
		}
	}
	...
	else { $result = $db->query($query." ORDER BY date_added ASC LIMIT $start, $per_page"); }
	...
	if($orderby != "" && $orderby == "ASC")
	{
		...
		$show_title = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=title&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Title</a>';	
		$show_name = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=author&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Author</a>';		
		$show_price = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=prep&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Prep(in Min.)</a>';		
		$show_duration = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=cook&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Cook(in Min.)</a>';		
		$show_total_hours = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=servings&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Servings</a>';
		$show_hourly_cost = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=yield&order=DESC&search='.$search_txt.'" title="Click to Sort in desending order.">Yield</a>';
	...
	else
	{
		...
		$show_title = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=title&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Title</a>';	
		$show_name = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=author&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Author</a>';		
		$show_price = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=prep&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Prep(in Min.)</a>';		
		$show_duration = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=cook&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Cook(in Min.)</a>';		
		$show_total_hours = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=servings&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Servings</a>';
		$show_hourly_cost = '<a href="package-list.php?page=gclt&PageNo='.$pagenum.'&field=yield&order=ASC&search='.$search_txt.'" title="Click to Sort in desending order.">Yield</a>';
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        Recipe List
                        <small>Front-End Recipe List</small>
                    </h1>
                    <ol class="breadcrumb">
                                    ...
                                    <div class="row"  style="padding-top:10px; padding-bottom:10px;">
                                    	...
                                    	<div class="col-xs-6">
                                        	...
                                        	<div class="dataTables_info" id="example1_info">
												...
												<?php if($foundnum>0) { echo 'Showing  '.($start+1).' to '.($start+$per_page).' of '.$foundnum.' entries'; } else { echo '<strong style="color:#FF0000;">There is no recipe added yet.</strong>'; }?>
