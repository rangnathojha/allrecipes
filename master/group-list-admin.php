...
if(isset($_REQUEST["action"]))
{
	...
	{
		case "Inactive":
			$sql="UPDATE `groups_dummy` SET `status`='0' WHERE `id` in ($chkid)";
			$db->query($sql);			
			$msg='Status has been updated successfully to Pending!';
			$class='successmsg';
		break;
		case "Active":
			$sql="UPDATE `groups_dummy` SET `status`='1'  WHERE `id` in ($chkid)";
			$db->query($sql);			
		...
		case "Delete":			
			$sql="delete from `groups_dummy` where `id` in ($chkid)";
			$db->query($sql);
...
{
	$sql="UPDATE `groups_dummy` SET `sort_order`='".$_GET["rwsorder".$val]."'  WHERE `id` ='$val'";
	$db->query($sql);
...
?>
    ...
    end )as groupuser FROM `groups_dummy`  as t1 WHERE t1.id > 0 ";
								
								listpages($searcharray,$fieldtoshowlists,'group-list-admin.php',$query,'created_date','status','name','group-add-admin.php','There is no group added yet!',$sort_order=1);
								?>
